...
namespace goss {
  ...
  public:
    ...
    void eval(const double* states, double t, double* values)
    {
      ...
      const double cse_33 = exp(0.0748834069234172*states[0]);
      const double cse_34 = exp(cse_22);
      const double cse_35 = -1.0*states[0];
      const double cse_36 = -0.1*states[0];
      const double cse_37 = 1.0/(K_mKo + K_o);
      const double cse_38 = pow(states[10]/K_rb, N_rb);
      const double cse_39 = pow(states[11]/K_fb, N_fb);
      const double cse_40 = 1.0/(K_mpCa + states[11]);
      const double cse_41 = 1.0/(Ca_o + K_mCa);
      const double cse_42 = pow(cse_12, ncoop);
      const double cse_43 = pow(cse_12, mcoop);
      const double cse_44 = (cse_13*cse_13);
      const double cse_45 = pow(cse_44, 3/2);
      const double cse_46 = pow(cse_45, 4/3);
      ...
      const double cse_54 = exp(0.1691*states[0] - 5.495);
      const double cse_55 = exp(cse_17 + 6.7);
      const double cse_56 = exp(0.1*states[0] + 0.2);
      const double cse_57 = kbplus*cse_43;
      const double cse_58 = states[0] - 26.7081865284974*log(cse_0);
      const double cse_59 = exp(cse_22*(eta - 1.0));
      const double cse_60 = 1.0/(cse_33 - 1.0);
      const double cse_61 = states[0] - 26.7081865284974*log(Na_o*cse_2);
      const double cse_62 = exp(-0.2*states[0] - 6.7);
      const double cse_63 = 1.0/(pow(K_mNai*cse_2, 1.5) + 1.0);
      const double cse_64 = states[0] - 13.3540932642487*log(Ca_o/states[11]);
      const double cse_65 = 1.0/((K_mNa*K_mNa*K_mNa) + cse_1);
      const double cse_66 = exp(-0.0769230769230769*states[0] -
        0.153846153846154);
      ...
      const double cse_75 = 1.0/(1.0 +
        0.1245*exp(-0.00374417034617086*states[0]));
      ...
      const double cse_83 = 1.0/(exp(cse_35 - 40.0) + 1.0);
      ...
      const double cse_88 = 1.0/(cse_54 + exp(-0.0128*states[0] - 7.677));
      const double cse_89 = I_NaKMax*cse_37*cse_63*cse_75;
      const double cse_90 =
        -1.0*G_KpMax*cse_58/(exp(-0.167224080267559*states[0] +
        1.25217391304348) + 1.0);
      const double cse_91 = -G_KsMax*(states[5]*states[5])*(states[0] -
        26.7081865284974*log((K_o + 0.01833*Na_o)/(0.01833*Na_i +
        states[8])));
      const double cse_92 = PCa*cse_60*cse_74*states[0]*states[27]*states[28];
      const double cse_93 = Ca_o*exp(eta*cse_22)/(cse_2*cse_2*cse_2) +
        cse_1*cse_26*cse_59;
      const double cse_94 = K_SR*cse_85*cse_86;
      const double cse_95 =
        -0.5*G_KrMax*sqrt(K_o)*cse_58*states[4]/(1.4945*exp(0.0446*states[0])
        + 1.0);
      const double cse_96 = G_tiMax*cse_49*cse_58/((K_mK1 + K_o)*(pow(cse_0,
        -1.5)*exp(0.0561625551925629*states[0]) + 2.0));
      ...
      values[1] = cse_29*(cse_83*(3.56*exp(0.079*states[0]) +
        310000.0*exp(0.35*states[0])) + (-1.0*cse_83 +
        1.0)/(0.13*exp(-0.0900900900900901*states[0] - 0.96036036036036) +
        0.13)) + 0.135*cse_83*(cse_29 + 1.0)*exp(-0.147058823529412*states[0]
        - 11.7647058823529);
      values[2] =
        cse_31*(0.1212*cse_83*exp(-0.01052*states[0])/(exp(-0.1378*states[0]
        - 5.531292) + 1.0) + (-0.3*cse_83 +
        0.3)*exp(-2.535e-7*states[0])/(exp(cse_36 - 3.2) + 1.0)) +
        cse_83*(cse_31 + 1.0)*(states[0] +
        37.78)*(-127140.0*exp(0.2444*states[0]) -
        3.474e-5*exp(-0.04391*states[0]))/(exp(0.311*states[0] + 24.64053) +
        1.0);
      values[3] = (1.0 - 1.0/(exp(cse_35 - 90.0) +
        1.0))*(-0.08*states[3]*exp(-0.0909090909090909*states[0]) +
        (-states[3] + 1.0)*(fabs(states[0] + 47.13) <= 1.0e-6 ?
        1.0/(-0.005*states[0] - 0.13565) : (0.32*states[0] +
        15.0816)/(-exp(cse_36 - 4.713) + 1.0)));
      values[4] = (cse_54*cse_88 - states[4])/(1.0*cse_88 + 27.0);
      values[5] = (-states[5] + 1.0/(exp(-0.0735294117647059*states[0] +
        1.81617647058824) + 1.0))*(1.0*(7.19e-5*states[0] -
        0.000719)/(-exp(-0.148*states[0] + 1.48) + 1.0) +
        1.0*(0.000131*states[0] - 0.00131)/(exp(0.0687*states[0] - 0.687) -
        1.0));
      values[6] = -0.0989*states[6]*exp(-0.06237*states[0]) +
        0.04516*(-states[6] + 1.0)*exp(0.03577*states[0]);
      ...
      values[9] = 1.0*(cse_50 - cse_73)/(CSQNtot*KmCSQN*pow(KmCSQN +
        states[9], -2.0) + 1.0);
      ...
      values[11] = 1.0*(-HTRPNtot*cse_76 - LTRPNtot*cse_77 -
        ...
        10000.0*cse_97) + cse_48 - cse_94)/(cse_21*pow(KmCMDN + states[11],
        -2.0) + 1.0);
      values[12] = 1.0*(V_JSR*cse_73*cse_9 - V_myo*cse_48*cse_9 -
        0.0748834069234172*cse_19*cse_9*cse_92)/(cse_21*pow(KmCMDN +
        states[12], -2.0) + 1.0);
      ...
      values[28] = (-states[28] + 0.2 + 0.8/(exp(cse_17 + 2.5) + 1.0))/(20.0
        + 600.0/(exp(0.105263157894737*states[0] + 2.10526315789474) + 1.0));
      values[29] = cse_76;
      values[30] = cse_77;
