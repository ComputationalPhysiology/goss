...
void  ImplicitEuler:: attach(ODE* ode_){
  ode = ode_;
  //    ode->attach(this);
  justrefined=false;
  num_tsteps=0;
...
void ImplicitEuler:: forward(double* y, double t, double dt) {
  ...
  while (!done){
    ...
    step_ok = NewtonSolve(z1,prev,y,t+dt,dt,1.0);    
#ifdef DEBUG
...
#ifdef DEBUG
  printf("ImplicitEuler done with comp_jac = %d and rejected = %d at t=%1.2e in %ld steps\n",jac_comp,rejects,t, num_tsteps);
#endif
