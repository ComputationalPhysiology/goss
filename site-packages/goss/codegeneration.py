...
_class_template = """namespace goss {{
  ...
  class {ModelName} : public ParameterizedODE, public LinearizedODE
  {{
  public:

    // Constructor
    {ModelName}() : ODE({num_states}),
      ParameterizedODE({num_states}, {num_parameters}, {num_field_states}, {num_field_parameters}, {num_monitored}), LinearizedODE({num_states}),
...
{eval_code}
    ...
    double eval(uint id, const double* states, double t)
    {{
      switch (id) 
      {{
...
{jacobian_code}{factorizing_code}{fb_substitution_code}        
...
{linear_terms}        
    ...
    // Eval linearized derivatives
    void linear_derivatives(const double* states, double t, double* values) const
    {{
...
_class_form = dict(
  ...
  eval_componentwise_code="NOT_IMPLEMENTED",
  linear_terms="NOT_IMPLEMENTED",
  linear_derivatives_code="NOT_IMPLEMENTED",
  jacobian_code="",
  factorizing_code="",
...
class GossCodeGenerator(CppCodeGenerator):
    ...
    def _eval_componentwise_code(self):
        body = self.dy_componentwise_body(parameters_in_signature=False)
        code = "\n".join(self.indent_and_split_lines(body, indent=3))
        self.class_form["eval_componentwise_code"] = code
    ...
    def _eval_linearized_code(self):
        ...
        code = "\n".join(self.indent_and_split_lines(body, indent=3))
        self.class_form["linear_terms"] = code

        body = self.linearized_dy_body(result_name="values")
        code = "\n".join(self.indent_and_split_lines(body, indent=3))
        self.class_form["linear_derivatives_code"] = code
        
    def _jacobian_code(self):
        body = self.jacobian_body(parameters_in_signature=False, \
                                  result_name="jac")
        body = self.wrap_body_with_function_prototype(\
            body, "compute_jacobian", "double t, double* states, double* jac",
    ...
    def _factorizing_jacobian_code(self):
        ...
        body = ["", "// Symbolically factorizing jacobian"]
    ...
    def _fb_substitution_code(self):
        ...
        body = ["// Copy b to dx", "for (unsigned int i=0; i<_num_states; i++)"]
