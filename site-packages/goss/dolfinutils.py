        ...
        for label in distinct_domains:
            ...
            for vert in (d.SubsetIterator(domains, label) \
                ...
                for local_dof_offset in range(num_field_states):
                    ...
                    if first_dof <= local_dof < last_dof:
            ...
            for names, value in dof_maps[label].items():
                dof_maps[label][names] = np.array(sorted(value), dtype=np.intc)
            
            # Allocate memory for the field values
            values[label] = np.zeros(sum(len(value) for value in \
                                         dof_maps[label].values()), dtype=np.float_)

        # Do we need nested dofs? 
        self._nested_dofs = len(distinct_domains) > 1 or num_fields_states > 1

        if self._nested_dofs:
            values["dolfin"] = np.zeros(sum(num_field_states*num_dofs[label] \
                                            for label in distinct_domains), \
                                        dtype=np.float_)
        else:
            # Grab the data used to update ODESystemSolver
            values["dolfin"] = values.values()[0]
