    ...
    def __init__(self, mesh, odes, domains=None, space="P1", params=None):
        ...
        for label in distinct_domains:
            ...
            if hasattr(ode, "field_params") and len(ode.field_params)>0:
                ...
                vertex_to_dof_param = d.vertex_to_dof_map(V_param)

                # Get dofs local to geometric domain
                dofs = vertex_to_dof_param[vertices[label]]
                dofs = dofs[(0<=dofs)*(dofs<num_local_param_dofs)]# + first_dof
                #dofs += first_dof
