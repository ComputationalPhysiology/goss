...
def family_and_degree_from_str(space):
    ...
    assert len(space) in [2,3], "expected 'space' to be P1, Q{1,2,...,9}"\
           " or DG{0,1,...,9}"

    if space[0] == "P":
        assert len(space)==2 and space[1]=="1", "expected only P1 spaces"
        family = "Lagrange"
        degree = 1
    elif space[0] == "Q":
        assert len(space)==2 and space[1] in "123456789", "expected only "\
               "Quadrature spaces with degree 1-9."
        family = "Quadrature"
        degree = int(space[-1])
    elif len(space)==3 and space[:2] == "DG":
        assert space[-1] in "0123456789", "expected only "\
                   "discontinuous spaces with degree 0-9."
        family = "Discontinuous Lagrange"
        degree = int(space[-1])
    else:
        assert False, "expected 'space' to be P1, Q{1,2,...,9}"\
               " or DG{0,1,...,9}"
...
class DOLFINODESystemSolver(object):
    ...
    def __init__(self, mesh, odes, domains=None, space="P1", params=None):
        """
        Arguments:
        ----------
        mesh : Mesh
