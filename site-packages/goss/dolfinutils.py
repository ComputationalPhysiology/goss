...
class DOLFINODESystemSolver(object):
    ...
    def __init__(self, mesh, odes, domains=None, params=None):
        ...
        if hasattr(solver, "set_newton_tol"):
            solver.set_newton_tol(self.params["newton_tol"])
        if hasattr(solver, "set_absolute_tol"):
            solver.set_absolute_tol(self.params["absolute_tol"])
        ...
        for label in distinct_domains:
            ...
            if hasattr(ode, "field_params") and len(ode.field_params)>0:
                ...
                dofs = dofs[(0<=dofs)*(dofs<num_local_dofs)]
            
            # Init memory for setting field_parameters
            param_values = np.zeros(\
                num_dofs[label]*ode.num_field_parameters(), dtype=np.float_)
    ...
    def default_parameters():
        ...
        params.add("scheme", "ImplicitEuler", goss_solvers)
        params.add("num_threads", 0, 0, 100)
        params.add("newton_tol", 1e-6, 1e-20, 1e-3)
        params.add("absolute_tol", 1e-6, 1e-20, 1e-6)
