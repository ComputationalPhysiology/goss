...
class GossCodeGenerator(CppCodeGenerator):
    ...
    def __init__(self, ode, field_states=None, field_parameters=None,
        ...
        code_params.default_arguments = "st"
        code_params.time.name = "time"

        code_params.array.index_format = "[]"
        code_params.array.index_offset = 0
        code_params.array.flatten = True

        code_params.parameters.representation = "named"

        code_params.states.representation = params.state_repr
        code_params.states.array_name = "states"

        code_params.body.array_name = "body"
        code_params.body.representation = params.body_repr
        code_params.body.use_cse = params.use_cse
        code_params.body.optimize_exprs = params.optimize_exprs

        # Init base class
        super(GossCodeGenerator, self).__init__(code_params)
    ...
    def _variable_init_and_declarations(self):
        ...
        ic_code.extend("values->data[{0}] = {1}".format(\
            i, state.init) for i, state in enumerate(ode.states))
    ...
    def _eval_linearized_code(self):
        ...
        lin = linearized_derivatives(self.ode, result_name="values", \
                                     params=self.params)
        
        body = ["indices[{0}] = {1}".format(ind, int(linear)) \
                for ind, linear in enumerate(lin.linear_derivative_indices)]
        body = self.wrap_body_with_function_prototype(\
        ...
        body.extend(self.function_code(lin, return_body_lines=True))
        body = self.wrap_body_with_function_prototype(\
    ...
    def _jacobian_code(self):
        ...
        return 
        fact = factorized_jacobian_expressions(jac, params=self.params)
        ...
        fb_subst = forward_backward_subst_expressions(factorized, \
                                                      params=self.params)
