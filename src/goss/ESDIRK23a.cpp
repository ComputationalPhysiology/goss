...
bool ESDIRK23a::do_step(double* y, const double& t0, const double& delta_t)
{
  ...
  if (step_ok){
    for (i = 0; i< num_states(); i++){
      z4[i] = z3[i];
      _prev[i] = z1[i]*b1+z2[i]*b2+z3[i]*b3;//y[i]+delta_t*(z1[i]*b1+z2[i]*b2+z3[i]*b3);
    }
     ...
     const double& dt)
{
  ...
  std::vector<double> y_int(num_states()), z_int(num_states()),
    dz(num_states()), prev_dz(num_states());
